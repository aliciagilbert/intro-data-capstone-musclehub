...
fitness_tests.fitness_test_date, applications.application_date, purchases.purchase_date from 
visits left join fitness_tests 
...
#here count first_names in each group
...
plt.pie(ab_counts.first_name.values, autopct='%0.2f%%', labels=['A','B'])
plt.axis('equal')
plt.show()
...
print(pval)
#Group B (no fitness test) was significantly higher than group A in turning in applications.
...
chi2, pval, dof, expected = chi2_contingency(X)
print(pval)
#Of those who picke up an application, group A (fitness test) was NOT significantly higher than group B in 
#purchasing a membership.
#Thus if they picked up an application whether fitness test or not, they were equally likely to purchase a membership.
...
#More people who did not take fitness test purchased a membership
...
chi2, pval, dof, expected = chi2_contingency(X)
print(pval)
#Since p<0.05 the difference is significant.  So significanly more people who did not take the fitness test purchased a membership.
